...
void CycleInfo::_ready() {
	...
	this->balance->set_text(_balance_to_string(gameState->get_balance()));
...
void CycleInfo::_display_cycle(const int month, const int year, const real_t balance) {
	...
	this->balance->set_text(_balance_to_string(balance));

	// Reset actions cost
	_display_actions_cost(0.f);
}

String CycleInfo::_balance_to_string(const real_t balance) {
	std::stringstream balance_str;
	balance_str << std::fixed << std::setprecision(2) << balance;
	return String(balance_str.str().c_str()) + String(" $");
}
